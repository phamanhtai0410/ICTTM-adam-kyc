...
import { TitleCapiralize, allCountriesList } from 'helpers'
import { DetailsModalButton } from '../Modal/DetailsModalButton/DetailsModalButton'
import assets from '../../../../assets/index'

export function DetailsCompanyTable ({ data, schema }) {
  return (
    <div className='person-details__relationship person-details__factsheet'>
      <div className='person-details__factsheet-column person-details__sanction-column'>
        <div className='person-details__relationship-column__hint'><p className='person-details__media-column__subtext'>{schema === 'CryptoWallet' ? 'Wallet holder' : 'Issuer'}</p></div>
      </div>
      ...
      {data?.map((item, index) => {
        return (
          <div className='person-details__factsheet-column' key={`${item} ${index}`}>
            ...
            <div className='person-details__relationship-column__4'>
              <p>{Array.isArray(item.properties.status) ? item.properties.status.map(TitleCapiralize).join(', ') : TitleCapiralize(item.properties.status) || '-'}</p>
            </div>
            <div className='person-details__media-column__5 person-details__relationship-column__5'>
              <DetailsModalButton item={item} type='Wallet holder' />
            </div>
          </div>
        )
      })}
